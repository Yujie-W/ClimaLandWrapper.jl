...
function dp_dz!(p, params, z)
    ...
    R_m = gas_constant_air(thermo_params, q)
    grav = grav(thermo_params)

    if thermo_flag == "θ_liq_ice"
        θ_liq_ice = prof_thermo_var(z)
        _cp_m = cp_m(thermo_params, q)
        _MSLP = MSLP(thermo_params)
...
function p_ivp(::Type{FT}, params, p_0, z_0, z_max) where {FT}
    ...
    z_span = (z_0, z_max)
    prob = ODEProblem(dp_dz!, p_0, z_span, params)
...
function surface_reference_thermo_state(::Soares, thermo_params)
    ...
    qtg::FT = 5.0e-3
    Tg::FT = 300.0
    return PhaseEquil_pTq(thermo_params, Pg, Tg, qtg)
end


function initialize_profiles(::Soares, grid::Grid, thermo_params, state; kwargs...)
    aux_gm = center_aux_grid_mean(state)

    FT = float_type(state)

    # Read in the initial profiles
    prof_q_tot = Soares_q_tot(FT)
    prof_θ_liq_ice = Soares_θ_liq_ice(FT)
    prof_u = Soares_u(FT)
    prof_tke = Soares_tke(FT)

    # Solve the initial value problem for pressure
    p_0::FT = FT(1000 * 100) # TODO - duplicated from surface_reference_thermo_state
    z_0::FT = grid.zf[kf_surface(grid)].z
    z_max::FT = grid.zf[kf_top_of_atmos(grid)].z
...
function surface_params(case::Soares, surf_ref_thermo_state, thermo_params)
    p_f_surf = air_pressure(thermo_params, surf_ref_thermo_state)
    ρ_f_surf = air_density(thermo_params, surf_ref_thermo_state)
    FT = eltype(p_f_surf)
    ...
    θ_flux::FT = 6.0e-2
    qt_flux::FT = 2.5e-5
    ts = PhaseEquil_pTq(thermo_params, p_f_surf, Tsurface, qsurface)
    lhf = qt_flux * ρ_f_surf * latent_heat_vapor(thermo_params, ts)
    shf = θ_flux * cp_m(thermo_params, ts) * ρ_f_surf
    return FixedSurfaceFlux(zrough, ts, shf, lhf)
end
...
function initialize_profiles(
    ...
    prof_θ_liq_ice = Nieuwstadt_θ_liq_ice(FT)
    prof_u = Nieuwstadt_u(FT)
    prof_tke = Nieuwstadt_tke(FT)
    prof_q_tot = nothing
...
function initialize_profiles(
    ...
    # Read in the initial profiles
    prof_q_tot = Bomex_q_tot(FT)
    prof_θ_liq_ice = Bomex_θ_liq_ice(FT)
    prof_u = Bomex_u(FT)
    prof_tke = Bomex_tke(FT)
...
function surface_params(case::Bomex, surf_ref_thermo_state, thermo_params)
    ...
    θ_flux::FT = 8.0e-3
    qt_flux::FT = 5.2e-5
    ts = PhaseEquil_pθq(thermo_params, p_f_surf, θ_surface, qsurface)
    Tsurface = air_temperature(thermo_params, ts)
    lhf = qt_flux * ρ_f_surf * latent_heat_vapor(thermo_params, ts)
    shf = θ_flux * cp_m(thermo_params, ts) * ρ_f_surf
    ustar::FT = 0.28 # m/s
    return FixedSurfaceFluxAndFrictionVelocity(zrough, ts, shf, lhf, ustar)
end
...
function initialize_profiles(
    ...
    prof_q_tot = LifeCycleTan2018_q_tot(FT)
    prof_θ_liq_ice = LifeCycleTan2018_θ_liq_ice(FT)
    prof_u = LifeCycleTan2018_u(FT)
    prof_tke = LifeCycleTan2018_tke(FT)
...
function surface_params(
    ...
    qt_flux::FT = 5.2e-5
    ts = PhaseEquil_pθq(thermo_params, p_f_surf, θ_surface, qsurface)
    lhf0 = qt_flux * ρ_f_surf * latent_heat_vapor(thermo_params, ts)
    shf0 = θ_flux * cp_m(thermo_params, ts) * ρ_f_surf
...
function surface_reference_thermo_state(::Rico, thermo_params)
    ...
    pvg = saturation_vapor_pressure(thermo_params, Tg, Liquid())
...
function initialize_profiles(
    ...
    prof_u = Rico_u(FT)
    prof_v = Rico_v(FT)
    prof_q_tot = Rico_q_tot(FT)
    prof_θ_liq_ice = Rico_θ_liq_ice(FT)
    ...
    # Fill in the grid mean values
    prog_gm_uₕ = grid_mean_uₕ(state)
    set_z!(prog_gm_uₕ, prof_u, prof_v)
    z = coordinate_field(axes(p_c)).z
    @. aux_gm.θ_liq_ice = prof_θ_liq_ice(z)
    @. aux_gm.q_tot = prof_q_tot(z)
    @. p_c = prof_p(z)
    ...
    @. aux_gm.θ_virt = virtual_pottemp(
        thermo_params,
        PhaseEquil_pθq(thermo_params, p_c, aux_gm.θ_liq_ice, aux_gm.q_tot),
    )
...
function surface_params(
    ...
    ts = PhaseEquil_pTq(thermo_params, p_f_surf, Tsurface, FT(0)) # TODO: is this correct?
    qsurface = q_vap_saturation(thermo_params, ts)
    # TODO: thermo state should be constructed once
    ts = PhaseEquil_pTq(thermo_params, p_f_surf, Tsurface, qsurface)
    return FixedSurfaceCoeffs(; zrough, ts, ch, cm)
end
...
function initialize_profiles(
    ...
    prof_T = TRMM_LBA_T(FT)
    prof_u = TRMM_LBA_u(FT)
    prof_v = TRMM_LBA_v(FT)
    prof_tke = TRMM_LBA_tke(FT)
    ...
    @. p_c = prof_p(z)
    @. aux_gm.q_tot = prof_q_tot(z)
    @. aux_gm.θ_liq_ice = liquid_ice_pottemp_given_pressure(
        thermo_params,
        prof_T(z),
        p_c,
        PhasePartition(aux_gm.q_tot, FT(0), FT(0)), # initial state is not saturated,
    )
    @. aux_gm.tke = prof_tke(z)
end
...
function initialize_profiles(
    ...
    # Load the initial profiles
    prof_u = ARM_SGP_u(FT)
    prof_q_tot = ARM_SGP_q_tot(FT)
    prof_θ_liq_ice = ARM_SGP_θ_liq_ice(FT)
    prof_tke = ARM_SGP_tke(FT)
    ...
    p_c = center_aux_grid_mean_p(state)
    set_z!(prog_gm_uₕ, prof_u, x -> FT(0))
    z = coordinate_field(axes(prog_gm_uₕ)).z
    # TODO figure out how to use ts here
    @. p_c = prof_p(z)
    @. aux_gm.q_tot = prof_q_tot(z)
    @. aux_gm.T =
        prof_θ_liq_ice(z) * exner_given_pressure(
            thermo_params,
            p_c,
            PhasePartition(aux_gm.q_tot, aux_gm.q_liq, FT(0)),
        )
    @. aux_gm.θ_liq_ice = liquid_ice_pottemp_given_pressure(
        thermo_params,
        aux_gm.T,
...
function initialize_profiles(
    ...
    # Load the initial profiles
    prof_q_tot = GATE_III_q_tot(FT)
    prof_T = GATE_III_T(FT)
    prof_tke = GATE_III_tke(FT)
    prof_u = GATE_III_u(FT)
    ...
    # Fill in the grid mean values
    prog_gm_uₕ = grid_mean_uₕ(state)
    set_z!(prog_gm_uₕ, prof_u, x -> FT(0)) # TODO: double check with CliMA Atmos folks to make sure this is the case
    @inbounds for k in real_center_indices(grid)
        ...
        aux_gm.θ_liq_ice[k] = liquid_ice_pottemp(thermo_params, ts)
    end
end
...
function surface_params(
    ...
    ts = PhaseEquil_pθq(thermo_params, p_f_surf, Tsurface, qsurface)
    return FixedSurfaceCoeffs(; zrough = FT(0), ts, ch, cm)
end
...
function surface_reference_thermo_state(::DYCOMS_RF01, thermo_params)
    ...
    qtg::FT = 9.0 / 1000.0
    θ_surf::FT = 289.0
    ts = PhaseEquil_pθq(thermo_params, Pg, θ_surf, qtg)
    Tg = air_temperature(thermo_params, ts)
    return PhaseEquil_pTq(thermo_params, Pg, Tg, qtg)
end
...
function initialize_profiles(
    ...
    # Load the initial profiles
    prof_u = Dycoms_RF01_u0(FT)
    prof_v = Dycoms_RF01_v0(FT)
    prof_q_tot = Dycoms_RF01_q_tot(FT)
    prof_θ_liq_ice = Dycoms_RF01_θ_liq_ice(FT)
    ...
    @inbounds for k in real_center_indices(grid)
        ...
        aux_gm.tke[k] = Dycoms_RF01_tke(FT)(z)
        p_c[k] = prof_p(z)
    end
end
...
function initialize_profiles(
    ...
    # Load the initial profiles
    prof_u = Dycoms_RF02_u(FT)
    prof_v = Dycoms_RF02_v(FT)
    prof_q_tot = Dycoms_RF02_q_tot(FT)
    prof_θ_liq_ice = Dycoms_RF02_θ_liq_ice(FT)
    ...
    @inbounds for k in real_center_indices(grid)
        ...
        # velocity profile
        aux_gm.tke[k] = Dycoms_RF02_tke(FT)(z)
        p_c[k] = prof_p(z)
    end
end
...
function initialize_profiles(
    ...
    prof_u = GABLS_u(FT)
    prof_v = GABLS_v(FT)
    prof_θ_liq_ice = GABLS_θ_liq_ice(FT)
    prof_q_tot = GABLS_q_tot(FT)
...
function surface_params(
    ...
    qsurface::FT = 0.0
    zrough::FT = 0.1
    ts = t -> PhaseEquil_pTq(thermo_params, p_f_surf, Tsurface(t), qsurface)
    return MoninObukhovSurface(; ts, zrough)
end
